...
selt () {
	...
	if [[ -z $fileList ]] ; then
		clear
		echo ; echo "No APK  files found. Coloca la APK que quieres modificar en la carpeta $dir1."
		#return 1
	else
...
actvfile () {
	...
	read INPUT
	if [[ x$INPUT == "xy" || x$INPUT == "xY" || x$INPUT == "x" ]]; then selt ; retval=$? ; if [[ $retval == 1 ]]; then
...
restart () {
	...
	echo "-|10)| Sign        APK.              | |25)|                                  |"
	echo "-|11)| Copiar apk desde el(Telefono) | |26)|                                  |"
